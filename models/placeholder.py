...
drivable, everything else is a static obstacle. The threshold is set
per-scan from the scan's own height percentile rather than a fixed absolute
z, so it works whether z=0 means "at the sensor" (real KITTI, ground around
-1.7m) or "at road level" (the shared mock data, ground around 0m) --
without that, a single hardcoded cutoff is correct for one convention and
silently wrong for the other. Cannot distinguish dynamic objects at all (no
learned class boundary yet) -- confidence is deliberately capped low here so
any consumer doing confidence-based filtering treats this appropriately as
a stand-in, not a real prediction.
"""
import os
import sys
import numpy as np
...
from class_mapping import DRIVABLE, STATIC_OBSTACLE
...
def classify(points: np.ndarray):
    ...
    returns: labels (N,) in {0, 1}, confidence (N,) filled with a flat low value
    """
    z = points[:, 2]
    ground_z = np.percentile(z, GROUND_PERCENTILE)
    labels = np.where(z <= ground_z + GROUND_BAND_M, DRIVABLE, STATIC_OBSTACLE).astype(np.uint8)
