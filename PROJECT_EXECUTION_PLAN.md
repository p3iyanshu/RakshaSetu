...
| Date | Day | Event |
|---|---|---|
| **2026-09-12 (Saturday)** | tomorrow | **HARD DEADLINE — every member pushes their current work to GitHub** (`https://github.com/p3iyanshu/RakshaSetu`), on their own `feature/<module>` branch, with a PR opened even if the module isn't finished |
| **2026-09-16 (Wednesday)** | in 5 days | **Internal hackathon** |
...
3. **Segment** — a deep learning model labels every point `drivable / static_obstacle / dynamic_object / ignore`.
...
|---|---|---|---|---|
| Member 1 — Segmentation | Substantial: data pipeline, class remapping, PointNet++ model, a critical kNN-to-ball-query bug fix, downsample-then-propagate inference. 4 commits on `feature/segmentation-model`. | **No** — branch not pushed, no PR open | Low (work exists, just not shared) | Push `feature/segmentation-model`, open a PR by Saturday |
...
| Member 4 — ROS 2 Integration | `ros2_ws/` does not exist yet. Note: `shared/schemas.py`, the contract this member owns, is already written and in use by everyone — the day-1 prerequisite is done, the ROS 2 wrapper work is not | **No** | **High** — integration cannot start without this | Scaffold `ros2_ws/` and push `feature/ros2-integration` by Saturday, even as stubs |
...
- [ ] Grid engine has a working ring-binning implementation and a reported adaptive-vs-uniform savings number, even on mock data
